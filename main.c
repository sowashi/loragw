...
void decrypt(uint8_t *payload, char *decryptmsg) {
    ...
    LoRaMacPayloadDecrypt(payload + appPayloadStartIndex, datasize, key, address, dir, sequenceCounter, LoRaMacRxPayload);

    for(int count=0; count < datasize; ++count) {
        //printf("0x%02x ", LoRaMacRxPayload[count]);
        if (LoRaMacRxPayload[count] == 0x00) {
            decryptmsg[count] = 0x00;
            break;
        }
        decryptmsg[count] = LoRaMacRxPayload[count];
    }
}
